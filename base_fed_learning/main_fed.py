...
def mnist_noniid_cluster_test(dataset, num_users, cluster):
    """
...
def gen_data(iid, dataset_type, num_users, cluster):
    ...
    if dataset_type == 'mnist':
        ...
        if iid:
            ...
            dict_test_users = mnist_noniid_cluster_test(dataset_test, num_users, cluster)
...
def FedMLAlgo(net_glob_list, w_glob_list, dataset_train, dict_users, num_users, clustering_matrix, multi_center_flag, dataset_test, cluster, cluster_length, dict_test_users, outputFile):
    ...
    if args.clustering_method == 'single':
        ...
        clustering_matrix, _, _, _ =\
            clustering_umap_central(args.num_users, dict_users, dataset_train, args.ae_model_name, 
                                        args.nr_epochs_sequential_training, cluster, args)
        plt.figure()
        plt.imshow(clustering_matrix)
